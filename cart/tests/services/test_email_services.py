...
from cart.services.email_services import send_email_to_owner, _build_email_message
...
class BuildEmailMessageTest(TestCase):
    ...
    def test_contains_customer_name(self):
        ...
        message = _build_email_message(
            'John Doe', 'john@example.com', 1, 'uuid-123', '2024-01-01',
            self.shipping_address, 'Oui', 'Non', 5.0, 100.0, 3.5, 108.5,
            self.list_products
        )
    ...
    def test_contains_order_id(self):
        ...
        message = _build_email_message(
            'John Doe', 'john@example.com', 42, 'uuid-123', '2024-01-01',
            self.shipping_address, 'Oui', 'Non', 5.0, 100.0, 3.5, 108.5,
            self.list_products
        )
    ...
    def test_handles_invalid_product(self):
        ...
        message = _build_email_message(
            'John Doe', 'john@example.com', 1, 'uuid-123', '2024-01-01',
            self.shipping_address, 'Non', 'Non', 5.0, 100.0, 0.0, 105.0,
            invalid_products
        )
...
class SendEmailToOwnerTest(TestCase):
    ...
    def test_handles_send_mail_exception(self):
        ...
        with patch('cart.services.email_services.send_mail', side_effect=Exception("SMTP error")):
            try:
                send_email_to_owner(**self.base_kwargs)
            except Exception:
    ...
    def test_insurance_is_oui_when_mandatory(self):
        ...
        with patch('cart.services.email_services.send_mail') as mock_send:
            with patch('cart.services.email_services._build_email_message') as mock_build:
                ...
                send_email_to_owner(**{**self.base_kwargs, 'total_articles_centimes': 6000})
