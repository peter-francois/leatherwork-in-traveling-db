...
def get_stripe_session(session_id: str):
    ...
    if session.payment_status != "paid":
        raise StripePaymentNotCompletedError("Payment not completed")

    metadata = getattr(session, "metadata", {})
    if not isinstance(metadata, dict) or "cart_uuid" not in metadata:
        raise StripeMetadataError("Invalid metadata or missing cart_uuid")
...
def extract_session_data(session, metadata) -> dict:
    ...
    shipping_address = (
        session.get("collected_information", {})
...
def _format_shipping_address(shipping_address: dict) -> dict:
    ...
    if line2 and line2.lower() != "none":
        shipping_address["formatted_shipping_address"] = ", ".join(
            filter(None, [shipping_address.get("line1", "Unknown"), line2])
        )
    else:
        shipping_address["formatted_shipping_address"] = shipping_address.get(
            "line1", "Unknown"
        )
