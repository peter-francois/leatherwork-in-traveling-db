...
class ProductFilterForm(forms.Form):
    search = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": _("Rechercher..."), "id": "search_field"}
        ),
    )
    product_type = forms.CharField(
        required=False, label=_("Type"), widget=forms.Select(choices=[("---", "---")])
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["product_type"].widget.choices = [("---", "---")] + [
            (t, t) for t in types
        ]
