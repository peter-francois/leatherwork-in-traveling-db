...
def get_product_images(request, article_id):
    try:
        ...
        return JsonResponse({'images': images, 'nom': product.name, 'description': product.description if product.description else None, 'discount': product.discount,'prix': product.price, 'en_attente_dans_panier': product.pending_in_cart, 'sur_commande': product.on_demand})
